...
import type { Category, CategoryInfo, Question } from '../../services/types'
...
export default function VideoImportPage() {
  ...
  const loadVideoInternal = useCallback(async (url: string) => {
    ...
    try {
      ...
      if (isPlaylistUrl) {
        ...
        if (playlistVideos.length === 0) {
          toast.error('Cette playlist ne contient aucune vidéo ou est inaccessible. Vérifiez que la playlist est publique.')
          setLoading(false)
          return
        }
        ...
        if (!metadata) {
          toast.error('Impossible de récupérer les informations de la vidéo. Vérifiez que l\'URL est correcte et que la vidéo est accessible.')
          setLoading(false)
          return
        }
  ...
  return (
    <div className="video-import-page">
                  ...
                  <div key={video.videoId} className={`video-item ${!video.selected ? 'unselected' : ''}`}>
                    ...
                    <div className="video-info">
                      ...
                      {!video.answer.trim() && video.selected && (
                        <div className="video-warning" title="Cette vidéo n'a pas de réponse et ne sera pas importée">
                          <span style={{ marginRight: '0.5rem', fontSize: '16px' }}>⚠️</span> Aucune réponse
                        </div>
                      )}
            ...
            <div className="video-import-footer">
              {isImporting && (
                <div className="import-progress">
                  ...
                  <span>Importation en cours... {importProgress.current}/{importProgress.total}</span>
                </div>
              )}
              <button 
              ...
              <button 
                ...
                disabled={isImporting || selectedCategories.length === 0 || selectedVideos.length === 0}
      ...
      <ConfirmDialog
        ...
        title={
          confirmState.type === 'clear' ? 'Effacer les vidéos' :
          confirmState.type === 'remove' ? 'Retirer la vidéo' :
          confirmState.type === 'import' ? 'Confirmer l\'import' :
          confirmState.type === 'replace' || confirmState.type === 'replaceVideo' ? 'Remplacer les vidéos' :
          'Confirmation'
        }
        message={confirmState.message}
        confirmText="Confirmer"
        cancelText="Annuler"
        variant={confirmState.type === 'clear' || confirmState.type === 'remove' ? 'warning' : 'info'}
